...
#      Reads mapping to >50 places are discarded. This keeps highly repetitive reads from swamping
...
# --outFilterMismatchNoverReadLmax 0.04 ==> Maximum allowed mismatch rate per read (4% of read length).
#      For ~100 bp reads, this allows ~4 mismatches per read. Tight enough to remove low-quality or
#      mismapped reads, but not so strict that natural variation or sequencing errors break mapping. Standard value.
